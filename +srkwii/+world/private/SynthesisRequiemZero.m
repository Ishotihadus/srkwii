function y = SynthesisRequiem(fs, temporal_positions, f0, vuv, spectrogram, band_aperiodicity, default_f0, seeds_signals)
...
excitation_signal = GetExcitationSignal(...
  temporal_positions, fs, f0, vuv, seeds_signals.pulse, seeds_signals.noise, band_aperiodicity, default_f0);

% Waveform generation based on the overlap-add method
y = GetWaveform(excitation_signal, spectrogram, temporal_positions, f0, fs);

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function excitation_signal = GetExcitationSignal(...
  temporal_positions, fs, f0, vuv, pulse_seed, noise_seed, band_aperiodicity, default_f0)
...
[pulse_locations_index, interpolated_vuv] = TimeBaseGeneration(temporal_positions, f0, fs, vuv, time_axis, default_f0);
...
noise_current_index = zeros(1, size(noise_seed, 2));

% Generation of the aperiodic component
for i = 1 : number_of_aperiodicities
  [noise, noise_current_index] = GenerateNoise(length(aperiodic_component), noise_seed, i, noise_current_index);
  aperiodic_component = aperiodic_component + (noise .* interpolated_aperiodicity(i, 1 : length(aperiodic_component))');
end

% Generation of the periodic component
for i = 1 : length(pulse_locations_index)
  if interpolated_vuv(pulse_locations_index(i)) <= 0.5 || interpolated_aperiodicity(1, pulse_locations_index(i)) > 0.999
    continue; end
  noise_size = ...
    sqrt(max(1, -pulse_locations_index(i) + pulse_locations_index(min(length(pulse_locations_index), i + 1))));
  output_buffer_index = max(1, min(length(time_axis), pulse_locations_index(i) + base_index));
  ...
  periodic_component(output_buffer_index) = periodic_component(output_buffer_index) + response;
end
excitation_signal = periodic_component + aperiodic_component;

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function response = GetOnePeriodicExcitation(number_of_aperiodicities, pulse_seed, aperiodicity, noise_size)
response = zeros(length(pulse_seed(:, 1)), 1);
for i = 1 : number_of_aperiodicities
  response = response + pulse_seed(:, i) * (1 - aperiodicity(i)); end
response = response * noise_size;

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function y = GetWaveform(excitation_signal, spectrogram, temporal_positions, f0, fs)
y = zeros(length(excitation_signal), 1);
fft_size = (size(spectrogram, 1) - 1) * 2;
latter_index = fft_size / 2 + 1 : fft_size;

frame_period_sample = round((temporal_positions(2) - temporal_positions(1)) * fs);
...
for i = 2 : length(f0) - 2
  ...
  spectrum = exp(ifft(tmp_complex_cepstrum) / 2);
...
excitation_signal = flip(y);
y = zeros(length(excitation_signal), 1);

for i = length(f0) - 2 : -1 : 2
  origin = (i - 1) * frame_period_sample - half_win_len;
  safe_index = min(length(y), origin : origin + win_len - 1);
  safe_index = length(y) - safe_index + 1;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [pulse_locations_index, vuv_interpolated] = TimeBaseGeneration(...
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function multi_aperiodicity = AperiodicityGenration(temporal_positions, band_aperiodicity, time_axis)
number_of_aperiodicities = size(band_aperiodicity, 1);
multi_aperiodicity = zeros(number_of_aperiodicities, length(time_axis));

for i = 1 : number_of_aperiodicities
  multi_aperiodicity(i, :) = interp1(temporal_positions, 10 .^ (band_aperiodicity(i, :) / 10), time_axis, 'linear');
end

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [n, current_index] = GenerateNoise(N, noise_seed, frequency_band, current_index)
noise_length = size(noise_seed, 1);

index = rem(current_index(frequency_band) : current_index(frequency_band) + N - 1, noise_length);
