...
% 1 ms frame shift.
basic_frame_period = option.basic_frame_period;
...
boundary_f0_list = adjusted_f0_floor * 2.0 .^...
  ((1 : ceil(log2(adjusted_f0_ceil / adjusted_f0_floor) * channels_in_octave)) / option.channels_in_octave);

% down-sampling to target_fs Hz
[y, actual_fs] = GetDownsampledSignal(x, fs, target_fs);
fft_size = 2 ^ ceil(log2(length(y) + 5 + 2 * floor(actual_fs / boundary_f0_list(1) * 2)));
y_spectrum = fft(y, fft_size);

raw_f0_candidates = GetRawF0Candidates(length(basic_temporal_positions), boundary_f0_list,...
  length(y), basic_temporal_positions, actual_fs, y_spectrum, f0_floor, f0_ceil);

[f0_candidates, number_of_candidates] = DetectOfficialF0Candidates(raw_f0_candidates);
f0_candidates = OverlapF0Candidates(f0_candidates, number_of_candidates, option.blur_frames);
...
[connected_f0, vuv] = FixF0Contour(f0_candidates, f0_scores, option);
smoothed_f0 = SmoothF0Contour(connected_f0, option.filter_b, option.filter_a);

if frame_period == basic_frame_period
  f0_parameter.temporal_positions = basic_temporal_positions;
  f0_parameter.f0 = smoothed_f0;
  f0_parameter.vuv = vuv;
else
  temporal_positions = 0 : frame_period / 1000 : length(x) / fs;
  f0_parameter.temporal_positions = temporal_positions;
  f0_parameter.f0 = smoothed_f0(min(length(smoothed_f0) - 1, round(temporal_positions * 1000)) + 1);
  f0_parameter.vuv = vuv(min(length(smoothed_f0) - 1, round(temporal_positions * 1000)) + 1);
end
f0_parameter.f0_candidates = f0_candidates;
f0_parameter.f0_candidates_score = f0_scores;


% Step 6: 信頼度にもとづく基本周波数候補の除去（3.3.1 - 6）
function [new_f0_candidates, new_f0_scores] = RemoveUnreliableCandidates(f0_candidates, f0_scores)
...
% Step 4: 時間的なぼかし（3.3.1 - 4）
function new_f0_candidates = OverlapF0Candidates(f0_candidates, max_candidates, n)
if max_candidates == 0
...
new_f0_candidates = zeros(max_candidates * number_of_candidates, size(f0_candidates, 2));
...
% Step 1: 8 kHz に近いサンプリング周波数ダウンサンプリング、DC 成分の除去
...
% Step 2: 帯域通過フィルタによる基本波フィルタリング（3.3.1 - 2）
function raw_f0_candidates = GetRawF0Candidates(number_of_frames,...
  boundary_f0_list, y_length, temporal_positions, actual_fs, y_spectrum, f0_floor, f0_ceil)
...
% Step 3: 基本周波数候補の選定（3.3.1 - 3）
function [f0_candidates, number_of_candidates] = DetectOfficialF0Candidates(raw_f0_candidates)
...
% Step 2a: 各帯域での候補の選定
function f0_candidate = GetF0CandidateFromRawEvent(...
  boundary_f0, fs, y_spectrum, y_length, temporal_positions, f0_floor, f0_ceil)
filter_length_half = round(fs / boundary_f0 * 2);
band_pass_filter_base = nuttall(filter_length_half * 2 + 1);
shifter = cos(2 * pi * boundary_f0 * (-filter_length_half : filter_length_half)' / fs);
...
f0_candidate = GetF0CandidateContour(negative_zero_cross, positive_zero_cross, peak, dip, temporal_positions);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function f0_candidate = GetF0CandidateContour(negative_zero_cross, positive_zero_cross, peak, dip, temporal_positions)
usable_channel =...
...
if usable_channel > 0
  ...
  interpolated_f0_list(1, :) = interp1(negative_zero_cross.interval_locations, negative_zero_cross.interval_based_f0,...
    temporal_positions, 'linear', 'extrap');
  interpolated_f0_list(2, :) = interp1(positive_zero_cross.interval_locations, positive_zero_cross.interval_based_f0,...
    temporal_positions, 'linear', 'extrap');
  interpolated_f0_list(3, :) = interp1(peak.interval_locations, peak.interval_based_f0,...
    temporal_positions, 'linear', 'extrap');
  interpolated_f0_list(4, :) = interp1(dip.interval_locations, dip.interval_based_f0,...
    temporal_positions, 'linear', 'extrap');
...
negative_going_points = (1 : length(x))' .* (([x(2 : end) ; x(end)] .* x < 0) .* ([x(2 : end) ; x(end)] < x));

edge_list = negative_going_points(negative_going_points > 0);
fine_edge_list = edge_list - x(edge_list) ./ (x(edge_list + 1) - x(edge_list));
event_struct.interval_locations = (fine_edge_list(1 : end - 1) + fine_edge_list(2 : end)) / 2 / fs;
event_struct.interval_based_f0 = fs ./ diff(fine_edge_list);


% Step 7: 基本周波数軌跡の推定（3.3.2）
function [f0, vuv] = FixF0Contour(f0_candidates, f0_scores, option)
f0_base = SearchF0Base(f0_candidates, f0_scores);
f0_step1 = FixStep1(f0_base, option.jump_allowed_range);
f0_step2 = FixStep2(f0_step1, option.voice_range_minimum);
f0_step3 = FixStep3(f0_step2, f0_candidates, option.v_extend_allowed_range,...
  option.v_extend_max_length, option.v_extend_min_period, option.v_extend_max_search, f0_scores);
f0 = FixStep4(f0_step3, option.uv_threshold);
vuv = f0;
vuv(vuv ~= 0) = 1;
...
for i = 1 : length(boundary_list) / 2
  ...
  if distance < voice_range_minimum
    f0_step2(boundary_list((2 * i) - 1) : boundary_list(2 * i)) = 0; end
end

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Step 3: Voiced sections are extended based on the continuity of F0 contour
function f0_step3 = FixStep3(f0_step2, f0_candidates, allowed_range,  threshold1, threshold2, threshold3, f0_scores)
f0_step3 = f0_step2;
...
count = 0;
for i = 1 : length(boundary_list) / 2
  [extended_f0, tmp_range(2)] = ExtendF0(multi_channel_f0(i, :), boundary_list(i * 2),...
    min(length(f0_step2) - 1, boundary_list(i * 2) + threshold1), 1, f0_candidates, allowed_range, threshold3);
  [tmp_f0_sequence, tmp_range(1)] = ExtendF0(extended_f0, boundary_list((i * 2) - 1),...
    max(2, boundary_list((i * 2) - 1) - threshold1), -1, f0_candidates, allowed_range, threshold3);
...
if count > 0
  f0_step3 = MergeF0(multi_channel_f0, range, f0_candidates, f0_scores); end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function f0 = MergeF0(multi_channel_f0, range, f0_candidates, f0_scores)
...
function [merged_f0, new_ed] = MergeF0Sub(f0_1, st1, ed1, f0_2, st2, ed2, f0_candidates, f0_scores)
merged_f0 = f0_1;
...
score2 = 0;
for i = st2 : ed1
  score1 = score1 + SerachScore(f0_1(i), f0_candidates(:, i), f0_scores(:, i));
  score2 = score2 + SerachScore(f0_2(i), f0_candidates(:, i), f0_scores(:, i));
end
if score1 > score2
...
for i = 1 : length(f0_candidates)
  if f0 == f0_candidates(i) && score < f0_scores(i)
    score = f0_scores(i); end
end

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [extended_f0, shifted_origin] = ExtendF0(f0, origin, last_point, shift, f0_candidates, allowed_range, threshold)
extended_f0 = f0;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [best_f0, best_error] = SelectBestF0(reference_f0, f0_candidates, allowed_range)
best_f0 = 0;
...
% Step 5: 瞬時周波数を用いた候補の修正と信頼度の計算（3.3.1 - 5, 6）
function [new_f0_candidates, f0_scores] = RefineCandidates(x, fs, temporal_positions, f0_candidates, f0_floor, f0_ceil)
new_f0_candidates = f0_candidates;
...
for i = 1 : length(temporal_positions)
  for j = 1 : size(f0_candidates, 1)
    ...
    if tmp_f0 == 0
      continue; end
    [new_f0_candidates(j, i), f0_scores(j, i)] = GetRefinedF0(x, fs, temporal_positions(i), tmp_f0, f0_floor, f0_ceil);
  end
end

function [refined_f0, refined_score] = GetRefinedF0(x, fs, current_time, current_f0, f0_floor, f0_ceil)
...
numerator_i = real(spectrum) .* imag(diff_spectrum) - imag(spectrum) .* real(diff_spectrum);
power_spectrum = abs(spectrum) .^ 2;
instantaneous_frequency = frequency_axis + numerator_i ./ power_spectrum * fs / 2 / pi;
...
refined_f0 = sum(amplitude_list .* instantaneous_frequency_list) / sum(amplitude_list .* harmonics_index);

variation = abs(((instantaneous_frequency_list ./ harmonics_index) - current_f0) ./ current_f0);
...
% Step 8: 最終的な軌跡の確定と平滑化（3.3.3）
function smoothed_f0 = SmoothF0Contour(f0, b, a)
...
for i = 1 : length(boundary_list) / 2
  tmp_f0_contour = FilterF0Contour(multi_channel_f0(i, :), boundary_list((i * 2) - 1), boundary_list(i * 2), b, a);
