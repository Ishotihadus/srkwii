...
%
% Based on 0.2.3 (2017/01/02)

% set default parameters
f0_low_limit = 47;
fft_size = 2 ^ ceil(log2(4 * fs / f0_low_limit + 1));
threshold = option.threshold;
upper_limit = option.upper_limit;
frequency_interval = option.frequency_interval;
...
if isfield(f0_object, 'vuv')
  f0(f0_object.vuv == 0) = 0; end

number_of_aperiodicities = floor(min(upper_limit, fs / 2 - frequency_interval) / frequency_interval);
...
ap_debug = zeros(number_of_aperiodicities + 2, length(f0));
...
for i = 1 : length(f0)
  if D4CLoveTrain(x, fs, f0(i), temporal_positions(i), threshold) == 0
    ...
    ap_debug(:, i) = 20 * log10(1 - 0.000000000001);
    continue
  end
  current_f0 =  max(f0_low_limit, f0(i));
  coarse_aperiodicity = EstimateOneSlice(x, fs, current_f0,...
    frequency_interval, temporal_positions(i), fft_size, number_of_aperiodicities, window);
  coarse_aperiodicity = max(0, coarse_aperiodicity - (current_f0 - 100) * 2 / 100);
  ap_debug(:, i) = [-60; -coarse_aperiodicity(:); -0.000000000001]; % for debug;

  aperiodicity(:, i) = 10 .^...
    (interp1(coarse_axis, [-60; -coarse_aperiodicity(:); -0.000000000001], frequency_axis, 'linear') / 20);
end

source_object.aperiodicity = aperiodicity;
source_object.coarse_ap = ap_debug;
source_object.coarse_axis = coarse_axis;
source_object.frequency_axis = frequency_axis(:);
...
waveform = GetWindowedWaveform(x, fs, current_f0, current_position, 1.5, 2);
...
function coarse_aperiodicity = EstimateOneSlice(x, fs, current_f0,...
  frequency_interval, current_position, fft_size, number_of_aperiodicities, window)
if current_f0 == 0
  coarse_aperiodicity = zeros(number_of_aperiodicities, 1);
  return
end

static_centroid = GetStaticCentroid(x, fs, current_f0, current_position, fft_size);
waveform = GetWindowedWaveform(x, fs, current_f0, current_position, 2, 1);
smoothed_power_spectrum = GetSmoothedPowerSpectrum(waveform, fs, current_f0, fft_size);
static_group_delay = GetStaticGroupDelay(static_centroid, smoothed_power_spectrum, fs, current_f0, fft_size);
coarse_aperiodicity = GetCoarseAperiodicity(static_group_delay, fs, fft_size, frequency_interval, number_of_aperiodicities, window);

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 % 1: hanning, 2: blackman
function waveform = GetWindowedWaveform(x, fs, current_f0, current_position, half_length, window_type)
...
if window_type == 1 % hanning
  ...
  window = 0.08 * cos(pi * time_axis * current_f0 * 2) + 0.5 * cos(pi * time_axis * current_f0) + 0.42;
end
waveform = segment .* window - window * mean(segment .* window) / mean(window);

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function spectral_envelope = GetSmoothedPowerSpectrum(waveform, fs, f0, fft_size)
power_spectrum = abs(fft(waveform, fft_size)) .^ 2;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function centroid = GetStaticCentroid(x, fs, current_f0, current_position, fft_size)
waveform1 = GetWindowedWaveform(x, fs, current_f0, current_position + 1 / current_f0 / 4, 2, 2);
waveform2 = GetWindowedWaveform(x, fs, current_f0, current_position - 1 / current_f0 / 4, 2, 2);
...
centroid = -imag(weighted_spectrum) .* real(spectrum) + imag(spectrum) .* real(weighted_spectrum);

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function group_delay = GetStaticGroupDelay(static_centroid, smoothed_power_spectrum, fs, f0, fft_size)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function smoothed_group_delay = LinearSmoothing(group_delay, fs, fft_size, width)
...
low_frequency_replica =...
  interp1(f0 - low_frequency_axis, signal(frequency_axis < f0 + fs / fft_size), low_frequency_axis(:), 'linear', 'extrap');

signal(frequency_axis < f0) = low_frequency_replica(frequency_axis < f0) + signal(frequency_axis < f0);

signal(end : -1 : fft_size / 2 + 2) = signal(2 : fft_size / 2);

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function coarse_aperiodicity = GetCoarseAperiodicity(group_delay, fs, fft_size, frequency_interval, number_of_aperiodicities, window)
...
for i = 1 : number_of_aperiodicities
  ...
  cumulative_power_spectrum = cumsum(sort(power_spectrum(1 : fft_size / 2 + 1)));
  coarse_aperiodicity(i) = -10 * log10(cumulative_power_spectrum(fft_size / 2 - boundary) / cumulative_power_spectrum(end));
end
