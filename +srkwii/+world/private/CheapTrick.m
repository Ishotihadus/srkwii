...
%
% Based on 0.2.3 (2017/05/20)

default_f0 = option.default_f0;
q0 = option.q0;
q1 = option.q1;
dc_correction = option.dc_correction;

if isfield(option, 'fft_size') == 1
  fft_size = option.fft_size;
elseif isfield(option, 'f0_low_limit') == 1
  fft_size = 2 ^ ceil(log2(3 * fs / option.f0_low_limit + 1));
else
  fft_size = 2 ^ ceil(log2(3 * fs / 71 + 1));
end
...
if isfield(source_object, 'vuv')
  f0_sequence(source_object.vuv == 0) = default_f0; end

spectrogram = zeros(fft_size / 2 + 1, length(f0_sequence));
for i = 1:length(f0_sequence)
  if f0_sequence(i) < f0_low_limit
    f0_sequence(i) = default_f0; end
  spectrogram(:,i) = EstimateOneSlice(x, fs, f0_sequence(i), temporal_positions(i), fft_size, dc_correction, q0, q1);
end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function spectral_envelope = EstimateOneSlice(x, fs, current_f0, current_position, fft_size, dc_correction, q0, q1)
waveform = GetWindowedWaveform(x, fs, current_f0, current_position);
power_spectrum = GetPowerSpectrum(waveform, fs, fft_size, current_f0, dc_correction);
smoothed_spectrum = LinearSmoothing(power_spectrum, current_f0, fs, fft_size);
spectral_envelope = SmoothingWithRecovery(...
  [smoothed_spectrum; smoothed_spectrum(end - 1 : -1 : 2)], current_f0, fs, fft_size, q0, q1);

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function power_spectrum = GetPowerSpectrum(waveform, fs, fft_size, f0, dc_correction)
...
if dc_correction
  ...
  low_frequency_replica = interp1(f0 - low_frequency_axis,...
    power_spectrum(frequency_axis < f0 + fs / fft_size), low_frequency_axis(:), 'linear', 'extrap');
  power_spectrum(frequency_axis < f0) =...
    low_frequency_replica(frequency_axis < f0) + power_spectrum(frequency_axis < f0);
...
smoothed_spectrum = smoothed_spectrum + eps; % + abs(randn(length(smoothed_spectrum), 1)) * eps;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function spectral_envelope = SmoothingWithRecovery(smoothed_spectrum, f0, fs, fft_size, q0, q1)
quefrency_axis = (0 : fft_size - 1)' / fs;
smoothing_lifter = sin(pi * f0 * quefrency_axis) ./ (pi * f0 * quefrency_axis);
smoothing_lifter(fft_size / 2 + 2 : end) = smoothing_lifter(fft_size / 2 : -1 : 2);
smoothing_lifter(1) = 1;

compensation_lifter = q0 + 2 * q1 * cos(2 * pi * quefrency_axis * f0);
compensation_lifter(fft_size / 2 + 2 : end) = compensation_lifter(fft_size / 2 : -1 : 2);
tandem_cepstrum = fft(log(smoothed_spectrum));
tmp_spectral_envelope = exp(real(ifft(tandem_cepstrum .* smoothing_lifter .* compensation_lifter)));
